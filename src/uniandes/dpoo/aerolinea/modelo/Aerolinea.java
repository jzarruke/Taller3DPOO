...
public class Aerolinea {
    ...
    public void cargarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException, InformacionInconsistenteException {
        // TODO implementar
    	CentralPersistencia persitencia = new CentralPersistencia();
    	persitencia.getPersistenciaAerolinea(tipoArchivo).cargarAerolinea(archivo, this);
    }
    ...
    public void salvarAerolinea( String archivo, String tipoArchivo ) throws TipoInvalidoException, IOException {
        // TODO implementar
    	CentralPersistencia persitencia = new CentralPersistencia();
    	persitencia.getPersistenciaAerolinea(tipoArchivo).salvarAerolinea(archivo, this);
    }
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception {
    	...
    	try {
	    	for (String codigo: rutas.keySet()) {
	    		if (codigo.equals(codigoRuta)) {
	    			ruta = rutas.get(codigo);
	    		}
	    	}
    	} catch (Exception e) {
    		System.out.println("La ruta de codigo:" + codigoRuta + ", no se encuentra registrada para la aerolinea.");
    	}
    	try {
			...
			System.out.println("El avión" + nombreAvion + "no se encuentra en esta aerolinea.");
		}
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception {
        // TODO Implementar el método
		for (Vuelo vuelo: vuelos) {
			if (vuelo.getRuta().equals(rutas.get(codigoRuta))) {
				return vuelo.venderTiquetes(clientes.get(identificadorCliente), null, cantidad);
			}
		}
        return -1;
    }
